...
public class Chapter01 extends Novel {
    ...
    public Chapter01() {
        chapters.add((Page) () -> {
            printline(CHARACTER_LIST.get("BLUE LADY"), "I understand that you have some streamlined method of issuing orders, correct?" +
            ...
            printline(CHARACTER_LIST.get("BLUE LADY"), "Move me with the arrow keys.  That's simple enough, right?" +
                    "  Attack with the spacebar.  Defend with the x button.  The Enter button opens up the menu.");
            printline(CHARACTER_LIST.get("BLUE LADY"), "If my HP falls to 0, this body will wither away." +
        ...
        chapters.add((Page) () -> {
            if (choices.equals("Yes")) {
                printline(CHARACTER_LIST.get("BLUE LADY"), ActorSprite.SpriteModeFace.HAPPY, "Good.  Let's continue.");
            }
            else {
                pageIndex = -1;
                annoyance++;
                if (annoyance >= 5) {
                    printline(CHARACTER_LIST.get("BLUE LADY"), ActorSprite.SpriteModeFace.ANGRY, "....I can't tell if you are slow or if you are simply messing around.");
                    pageIndex = 1;
                }
            }
        });
